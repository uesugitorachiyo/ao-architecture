...
- AO Next is an independent experimental successor candidate, not a current AO Stack component. Its inventory entry and historical bounded-feasibility decision do not authorize succession, migration, providers, release, publication, deployment, promotion, or Mission-state ownership.
...
- Retain only bounded, digest-bound workflow evidence bytes required for independent parity. Apply environmental normalization only in the comparator under the separately hashed allowlist; never change the qualified source-baseline manifest merely to add comparison policy.
...
- Development-baseline parity changes: `python3 scripts/test_compare_development_baseline_results.py`, compare two distinct independently rehashed native results, then run the architecture verifier.
- Development-baseline evidence changes: `python3 scripts/test_verify_development_baseline_evidence.py`, validate one new downloaded proof root with `scripts/verify_development_baseline_evidence.py`, then run the architecture verifier.
